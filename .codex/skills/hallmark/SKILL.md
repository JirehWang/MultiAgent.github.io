---
name: hallmark
description: "Opt-in anti-AI-slop design skill for greenfield pages, audits, redesigns, and design extraction. Use only when the user explicitly invokes Hallmark or uses hallmark audit, hallmark redesign, or hallmark study; ordinary UI work stays with web-design-polish."
---
...
## Global integration boundary

- This is an opt-in creative structure and anti-slop layer. `web-design-polish` remains the default UI orchestrator.
- In an established project, `DESIGN.md` and existing design tokens are authoritative. Do not rotate themes or introduce a competing token system.
- Do not create `.hallmark/preflight.json` or `.hallmark/log.json` unless the user explicitly asks to enable Hallmark project memory.
- Use `visual-qa` for browser and responsive verification; Hallmark self-critique is not completion evidence.
...
**Persistence (opt-in).** Keep findings in the current run by default. Write `.hallmark/preflight.json` only when the user explicitly enables Hallmark project memory. When enabled, re-use the cached findings unless either:
...
- **Append to project memory only when enabled.** If the user explicitly enabled Hallmark project memory, update (or create) `.hallmark/log.json` at the project root after writing the stamp. Append a new entry at the **front** of the array: `{ "date": "<YYYY-MM-DD>", "macrostructure": "<name>", "theme": "<name>", "enrichment": "<E# name or 'none'>", "brief": "<one-line summary>" }`. **Custom entries** also carry `"theme": "custom"` plus `"theme_axes": "<paper-band> / <display-style> / <accent-hue>"` and an optional `"vibe": "<4–8 words>"` — see [`custom-theme.md`](references/custom-theme.md) § F. Trim the file to the last 20 entries. Without explicit approval, do not create or update `.hallmark/` files.
...
- **Emit or reuse design tokens.** For a greenfield project with no token system, write `tokens.css` at the project root containing the named tokens used in the build. In an established project, reuse its existing tokens and entry stylesheet; do not create a competing `tokens.css`. Page CSS references the authoritative project tokens by name and never inlines raw values. Load [`export-formats.md`](references/export-formats.md) only when the project warrants additional formats.
