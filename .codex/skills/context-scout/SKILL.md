...
to the discovery process itself. Read only enough context to move the work forward safely.
...
2. Check whether a narrow local search already answers the question before escalating to broader map tools.
3. If the repo is large, architectural boundaries are unclear, or the user asked for a codebase map, prefer `codebase_memory` before broad manual crawling.
4. If onboarding depends on remote PRs, issues, workflow runs, or repo state outside the local checkout, use `github`.
5. If onboarding starts from PDFs, docs, slides, or exported reports, use `markitdown` first to ingest the source material.
6. If the repo is large or boundaries are still unclear after that, use `Gitingest` for a high-level map.
7. Narrow to the few files or directories that matter.
8. If exact cross-file logic is still unclear, use `Repomix` only on that narrowed subtree or file set.
9. Read the real source and trace input -> module -> side effect.
10. Report only the context needed to move work forward:
...
- Do not turn onboarding into a full-repo excavation when the task can be solved from a smaller slice.
...
- Prefer the narrowest evidence-gathering path that safely answers the current question.
